...
- Replaced canonical `cbt_submit` with an original-index-aware, server-authoritative, idempotent scorer; `cbt_submit_v2` now delegates to it.
...
- Added scoring regression tests for MCQ letters/text, true/false, numeric tolerance and multi-select.
...
- Traditional build: 182 entries, 0 broken links, 0 orphans.
- Modern build: 385 entries, 0 broken links, 0 orphans.
