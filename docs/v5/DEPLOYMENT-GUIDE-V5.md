...
3. Run the full new `database/complete-schema.sql`. It is cumulative/idempotent and replaces the canonical CBT RPCs while adding compatibility columns/functions.
4. Confirm SQL functions exist:
   ```sql
   select routine_name from information_schema.routines
   where routine_schema='public'
     and routine_name in ('cbt_get_public_exam','cbt_submit','cbt_submit_v2','sc_cbt_answer_matches');
   ```
